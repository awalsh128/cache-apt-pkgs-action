...
#    - Works from both project root and scripts/dev/ directory
...
function __test_invalid_arguments() {
	local script_path="$1"
...
function __get_script_path_dynamic() {
	local test_filepath
	local root_path
	...
	script_filepath="${root_path}/scripts/dev/${script_filename}"
	...
	# Fallback: search scripts/dev/ for a matching script name
	if [[ -d "${root_path}/scripts/dev" ]]; then
		local found
		found=$(find "${root_path}/scripts/dev" -maxdepth 1 -type f -name "${script_filename}" -print -quit 2>/dev/null || true)
		if [[ -n ${found} ]]; then
